...
public class Casilla implements Comparable<Casilla> {
	...
	public static Casilla of(Integer p) {
		return new Casilla(p/DatosSudoku2.n,p%DatosSudoku2.n, null, false,false);
	}
	...
	private Casilla(Integer f, Integer c, Integer value, Boolean definida, Boolean intialValue) {
		super();
		Preconditions.checkArgument(0 <= f && f < DatosSudoku2.n, String.format("El numero de fila es %d",f));
		Preconditions.checkArgument(0 <= c && c < DatosSudoku2.n, String.format("El numero de columna es %d",c));
		this.f = f;
		this.c = c;
		this.value = value;	
	...
	public Integer st() {
		return this.c/DatosSudoku2.nct+DatosSudoku2.nft*(this.f/DatosSudoku2.nft);
	}
	
	public Integer p() {
		return this.f*DatosSudoku2.n+this.c;	
	}
